...
export function ListingDetail({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    fetchHomesListings()
      .then((listings) => {
        const match = listings.find((l) => l.slug === slug)
  ...
  if (notFound || !listing) {
    return (
      <div className="py-16 text-center">
        <p className="mb-4 text-gray-500">This listing is no longer available.</p>
        <Link href="/listings" className="text-brand-gold hover:underline">
          View all listings
        </Link>
      </div>
    )
    ...
    <>
      {images.length > 0 ? (
        <div className="mb-8 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
          {images.slice(0, 6).map((src, i) => (
            <div
              key={i}
              className={`relative overflow-hidden rounded-xl bg-gray-100 ${i === 0 ? 'aspect-[4/3] sm:col-span-2 lg:col-span-2' : 'aspect-[4/3]'}`}
            >
      ...
      <div className="grid gap-10 lg:grid-cols-3">
        <div className="flex flex-col gap-6 lg:col-span-2">
          ...
          <div>
            <h1 className="text-2xl font-bold text-brand-dark sm:text-3xl">{address.street}</h1>
