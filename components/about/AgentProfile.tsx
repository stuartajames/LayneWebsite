...
export function AgentProfile() {
  ...
  useEffect(() => {
    sanityClient
      .fetch(`*[_type == "agentProfile"][0]`)
      .then((doc) => {
        if (!doc) return
        setProfile({
          name: doc.name as string,
          bio: doc.bio as string,
          photo: doc.photo ? builder.image(doc.photo).width(800).format('webp').url() : '',
  ...
  const bioParas = profile?.bio ? profile.bio.split('\n').filter(Boolean) : FALLBACK_BIO
  ...
  return (
    <div className="grid gap-12 lg:grid-cols-2">
      ...
      <div className="flex flex-col gap-6">
        <div className="relative aspect-[3/4] max-w-sm overflow-hidden rounded-2xl bg-gray-100">
          {!loading && (
            <Image
              src={photoSrc}
      ...
      <div className="flex flex-col gap-8">
        ...
        <div>
          <h2 className="mb-3 font-semibold text-brand-dark">Area of expertise</h2>
            ...
            >
              ...
              {SUBURBS.map(({ name, x, y }) => (
                <g key={name}>
                  ...
                  <text x={x + 8} y={y + 4} fontSize="9" fill="#374151" fontFamily="system-ui, sans-serif">
                    {name}
                  </text>
                </g>
              ))}
              <text x="185" y="270" fontSize="10" fill="#9ca3af" fontFamily="system-ui, sans-serif" textAnchor="middle">
                Wellington CBD
              </text>
            </svg>
          </div>
